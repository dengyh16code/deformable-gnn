...
from models import Attention, Transport, TransportGoal
import cameras
import utils
...
class TransporterAgent:
    ...
    def __init__(self, name, task, num_rotations=24, crop_bef_q=True,
        ...
        if task in ["rope_n","rope_v","ring_square"]:
    ...
    def train(self, dataset, num_iter, writer):
        ...
        for i in range(num_iter):
            if self.use_goal_image:
                obs, act, goal = dataset.random_sample()
            else:
                obs, act = dataset.random_sample()
            ...
            p0= utils.position_to_bound_pixel((act[0],act[1],act[2]),self.bounds, self.pixel_size)            
            p1 = utils.position_to_bound_pixel((act[3],act[4],act[5]),self.bounds, self.pixel_size)
            p0_theta = 0
            p1_theta = 0
    ...
    def test(self, dataset, num_iter):
        average_err_pick = 0
        average_err_place = 0
        for i in range(num_iter):
            if self.use_goal_image:
                ...
                obs, act = dataset.random_sample()          
            # Get  labels from data sample.      
            p0_truth= (act[0],act[1])
            p1_truth = (act[2],act[3])
            colormap, heightmap = self.get_heightmap(obs, self.camera_config)
            p0_position_truth = np.array(utils.bound_pixel_to_position(p0_truth, heightmap, self.bounds, self.pixel_size))
            p1_position_truth = np.array(utils.bound_pixel_to_position(p1_truth, heightmap, self.bounds, self.pixel_size))

            if self.use_goal_image:
                act_dic = self.act(obs,goal)
            else:
                act_dic = self.act(obs)

            p0_position = np.array(act_dic['params']['pose0'][0])
            p1_position = np.array(act_dic['params']['pose1'][0])

            p0_error = np.sqrt(np.sum((p0_position-p0_position_truth) ** 2))
            p1_error = np.sqrt(np.sum((p1_position-p1_position_truth) ** 2))

            average_err_pick += p0_error/num_iter
            average_err_place += p1_error/num_iter
        
        return  average_err_pick,average_err_place

    def act(self, obs, goal=None, debug_imgs=False,):
        ...
        p0_position = utils.bound_pixel_to_position(p0_pixel, heightmap, self.bounds, self.pixel_size)
        p1_position = utils.bound_pixel_to_position(p1_pixel, heightmap, self.bounds, self.pixel_size)
        ...
        if debug_imgs:
            ...
            extras = {
                ...
                'inference_time':end_2-start_2 + end_1 - start_1
            }
    ...
    def visualize_images(self, p0, p0_theta, p1, p1_theta, original_pixels,
        ...
        plt.tight_layout()
        test_png_name = os.path.join(self.models_dir, "test.jpg")
...
class OriginalTransporterAgent(TransporterAgent):
    ...
    def __init__(self, name, task, num_rotations=24, crop_bef_q=True):
        ...
        '''
        flops_1 = get_flops(self.attention_model.model,batch_size=1)
        flops_2 = get_flops(self.transport_model.model,batch_size=1)
        self.attention_model.model.summary()
        #self.transport_model.model.summary()
        print("flops_total:",flops_1+flops_2)
        exit(0)
        '''
